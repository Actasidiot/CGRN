    ...
    with open(train_path, 'wb') as ft:
        with open(val_path, 'wb') as fv:
            for p in paths:
                ...
                font_label = os.path.basename(p).split("_")[0]
                char_label = int(os.path.basename(p).split("_")[1])
                img_name = os.path.basename(p).split("_")[2]
                with open(p, 'rb') as f:
                    print("img %s" % p, char_label)
                    img_bytes = f.read()
                    r = random.random()
                    example = (font_label, char_label, img_name, img_bytes)
